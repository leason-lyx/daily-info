...
- 可选摘要 provider：支持 OpenAI-compatible API 和 Codex CLI。
...
| `NEXT_PUBLIC_API_BASE_URL` | 浏览器访问后端 API 的地址，默认 `http://localhost:8000`。 |
...
| `LLM_API_KEY` | 可选的摘要 API key。 |
| `LLM_MODEL_NAME` | OpenAI-compatible provider 使用的模型名。 |
...
内置信息源定义位于 `config/sources/*.yaml`。应用启动时会把它们同步进数据库，作为可浏览的 catalog。
...
- 默认 AI provider 关闭；只有在配置 provider 后才会调用外部摘要服务。
