...
| `X_BEARER_TOKEN` | 可选的 X API v2 Bearer Token，用于 `x_user` 社交媒体 source。 |
| `X_API_BASE_URL` | 可选的 X API base URL，默认 `https://api.x.com/2`。 |
...
使用 `adapter: x_user` 的社交媒体 source 会通过官方 X API v2 读取公开 posts。把 `X_BEARER_TOKEN` 写入 `.env` 后，在 `/sources` 订阅对应 source 即可；worker 会保存最新 post id 作为 cursor，并在后续抓取时增量读取。
