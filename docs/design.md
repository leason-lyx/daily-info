...
- `/health`：运行状态和故障排查。
- `/settings`：数据库提示、RSSHub、AI provider、source pack 导入导出等配置。
...
DB 是运行时真源。YAML 仅用于 source pack 导入、导出、备份和分享。
...
`sources` 保存用户可编辑配置，例如名称、类型、平台、分组、轮询间隔、过滤词、默认标签、启停状态。
...
RSSHUB_PUBLIC_INSTANCES=https://rsshub.app,https://rsshub.rssforever.com
```
...
- 当前 `config/sources.yaml` 可作为初始 source pack 导入。
