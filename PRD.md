...
系统提供 built-in catalog，包含常见 AI 信息源，例如 arXiv、OpenAI、Anthropic、Google、Hugging Face、独立研究博客等。用户可以在 `/sources` 中一键启用、禁用或预览。
...
Source Registry 是系统的核心资产。重构后 DB 是运行时真源，YAML 是导入、导出、备份和分享格式。
...
2. 无 AI 配置时，系统可以启用内置源、抓取内容、浏览 feed、搜索和查看 health。
