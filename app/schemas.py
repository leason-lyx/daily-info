...
class FetchAttemptIn(BaseModel):
    adapter: Literal["feed", "rsshub", "html_index", "page_index", "x_user"]
    url: str = ""
    route: str = ""
    ...
    def require_location(self):
        ...
        if self.adapter == "x_user" and not (self.route or self.url):
            raise ValueError("x_user attempt requires route or url")
        return self
