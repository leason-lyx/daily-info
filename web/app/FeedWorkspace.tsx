...
export type FeedVisualVariant = "classic" | "swiss" | "bento" | "dark" | "archive";

type VisualVariant = Exclude<FeedVisualVariant, "classic">;

type FeedWorkspaceProps = {
  variant?: FeedVisualVariant;
  showStyleSwitcher?: boolean;
};

const VISUAL_VARIANTS: Array<{
  id: VisualVariant;
  number: string;
  label: string;
  href: string;
...
}> = [
  {
    id: "swiss",
    number: "01",
    label: "Swiss Editorial Academic",
    href: "/visual-directions/swiss",
    description: "Rational grid, thin rules, academic report rhythm.",
  },
  {
    id: "bento",
    number: "02",
    label: "Minimal Tech Bento",
    href: "/visual-directions/bento",
    description: "Soft SaaS surfaces, compact modules, light AI-tool mood.",
  },
  {
    id: "dark",
    number: "03",
    label: "Dark Futuristic Dashboard",
    href: "/visual-directions/dark",
    description: "Model-system console with restrained lab telemetry.",
  },
  {
    id: "archive",
    number: "04",
    label: "Warm Humanist Archive",
    href: "/visual-directions/archive",
    description: "Paper texture, quiet index cards, long-reading archive.",
  },
];

function variantMeta(variant: FeedVisualVariant) {
  return VISUAL_VARIANTS.find((item) => item.id === variant);
}
...
function FeedView({ variant = "classic", showStyleSwitcher = false }: FeedWorkspaceProps) {
  ...
  const statusText = isLoading && !items.length ? "Loading items..." : `${total} items match current filters`;

  return (
    <div className={`feedSurface feedSurface--${variant}`}>
      <header className="pageHead">
        <div>
          <h1>Unified Feed</h1>
          <span className="subtle">{meta ? `${meta.label} · ${statusText}` : statusText}</span>
        </div>
        {showStyleSwitcher && <StyleSwitcher activeVariant={variant} />}
      </header>
...
function StyleSwitcher({ activeVariant }: { activeVariant: FeedVisualVariant }) {
  return (
    <nav className="styleSwitcher" aria-label="Visual style switcher">
      {VISUAL_VARIANTS.map((item) => {
        const active = item.id === activeVariant;
        return (
          <Link
            className={`styleSwitch${active ? " active" : ""}`}
            href={item.href}
            key={item.id}
          ...
          >
            ...
            <strong>{item.label.replace(" Academic", "").replace(" Dashboard", "")}</strong>
          </Link>
        );
      })}
    </nav>
